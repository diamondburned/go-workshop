...
func (db *SQLite) Book(isbn bookstore.ISBN) (bookstore.Book, error) {
	book, err := db.q.GetBook(context.TODO(), string(isbn))
	if err != nil {
		return bookstore.Book{}, sqliteError(err)
	}
	return bookstore.Book{
		ISBN:   bookstore.ISBN(isbn),
		Title:  book.Title,
		Author: book.Author,
